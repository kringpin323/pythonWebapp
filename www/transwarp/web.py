...
class Request(object):
	...
	def __getitem__(self, key):   # get the attr like list
		'''
		Get input paramter value. If the specified key has multiple value, the first one is returned.
...
class Response(object):
	...
	def set_cookie(self, name, value, max_age=None, expires=None, path='/',domain=None, secure=False, http_only=True):
		...
		Args:
			...
			http_onluy: if the cookie is for http only, default to true for beeter satfy
							(client-side script cannot aceess cookies with httponly flag).
